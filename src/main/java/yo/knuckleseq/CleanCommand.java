...
final class CleanCommand {
    ...
    CleanCommand() {
        this(new CleanService());
    }

    CleanCommand(CleanService cleanService) {
    ...
    CommandResult execute(List<String> arguments) {
        ...
        try {
            ...
            return CommandResult.usageError(CliTexts.CLEAN_USAGE);
        }

        try {
            var config = AppConfig.load(options.configPath());
            return cleanService.execute(config, options.dreamId(), options.force());
            ...
            switch (argument) {
                ...
                default -> {
                    if (argument.startsWith("--") || dreamId != null) {
                        throw new IllegalArgumentException("Invalid clean arguments");
                    }
